# 🎬 Jimeng Seedance 2.0 사용자 매뉴얼(새로운 다중 모드 생성 경험)

텍스트와 첫 번째/마지막 프레임으로만 '이야기'를 전달할 수 있던 시절부터 우리는 사용자의 표정을 진정으로 이해하는 비디오 모델을 만들고 싶었습니다. 오늘, 정말 왔습니다!

Jimeng Seedance 2.0은 이제 이미지, 비디오, 오디오 및 텍스트의 네 가지 모달 입력 모드를 지원하며 더욱 풍부한 표현과 보다 제어 가능한 생성을 제공합니다.

사진을 사용하여 사진의 스타일을 설정하고, 비디오를 사용하여 캐릭터의 움직임과 렌즈 변경을 지정한 다음, 몇 초의 오디오를 사용하여 리듬과 분위기를 끌어낼 수 있습니다. 즉각적인 단어를 사용하면 창작 과정이 더욱 자연스럽고 효율적이며 실제 "감독"과 비슷해집니다.

이번 업그레이드에서는 "참조 기능"이 가장 큰 특징입니다.
- 📷 참조 이미지는 그림 구성과 문자 세부정보를 정확하게 복원할 수 있습니다.
- 🎥 참조 영상은 렌즈 언어, 복잡한 액션 리듬, 창의적인 특수 효과 재현을 지원합니다.
- ⏱ 영상은 원활한 확장과 연결을 지원하며, 사용자의 지시에 따라 연속적인 영상을 생성할 수 있습니다. 세대뿐 아니라 '계속 촬영'
- ✂️ 편집 기능이 동시에 향상되어 기존 영상의 역할 교체, 삭제, 추가를 지원합니다.

우리는 영상제작이 단지 '세대'에 관한 것이 아니라 표현의 통제에 관한 것임을 알고 있습니다. 2.0은 다중 모드일 뿐만 아니라 진정으로 제어 가능한 생성 방법이기도 합니다.

다중 모드 생성인 Seedance 2.0이 여기에서 시작됩니다. 과감하게 상상해보시고 나머지는 맡겨주세요

## 매개변수 미리보기

|핵심 차원| 시드스 2.0 |
| --- | --- |
| 이미지 입력 ​​및 형식: jpeg, png, webp, bmp, tiff, gif | ≤ 9개 사진 <br/> 크기: 30MB 미만 |
| 비디오 입력 및 형식: mp4, mov | ≤ 3, 총 재생 시간 [2,15]초 <br/> 크기: 50MB 미만<br/> 총 동영상 픽셀 범위: [409600(640×640,480p), 927408(834×1112,720p)] |
| 오디오 입력 및 형식: mp3, wav | ≤ 3, 총 지속 시간은 15초 이하<br/> 크기: 15MB 미만 |
| 텍스트 입력 | 자연어 |
| 생성 시간 | 15초 이하인 경우 4~15초를 자유롭게 선택할 수 있습니다 |
| 사운드 출력 | 음향 효과/사운드트랙 제공 |
| 상호 작용 제한: 현재 지원되는 최대 혼합 입력 수는 12개 파일입니다. 영상이나 리듬에 가장 큰 영향을 미치는 자료를 우선적으로 업로드하고, 다양한 모드에서 파일 수를 합리적으로 할당하는 것이 좋습니다 |

## ⚠️알림: 사실적인 얼굴 자료 업로드 지침
플랫폼 규정 준수 요구 사항으로 인해 실제 실제 얼굴이 포함된 자료 업로드는 현재 지원되지 않습니다(사진 및 비디오 자료 모두 허용되지 않음). 사용자 권리와 생성 보안을 보호하기 위해 시스템은 해당 자료를 자동으로 차단하며 업로드 후에는 비디오 콘텐츠가 생성되지 않습니다.
즉, 실제 사람의 사진(특히 식별 가능한 얼굴)을 업로드하는 경우 모델이 서신 생성을 처리할 수 없다는 의미입니다.
이로 인해 일부 제한이 발생할 수 있다는 점을 이해하지만, 이 조치는 콘텐츠 보안과 플랫폼 운영을 보장하기 위한 것입니다. 여러분의 이해와 협조에 감사드립니다! 앞으로 업데이트가 있으면 적시에 문서에 업데이트하겠습니다. 이해해 주셔서 다시 한번 감사드립니다~
...
# 다중 모드 종합 업그레이드: 비디오 제작이 "자유로운 조합" 시대로 들어갑니다!

## Seedance 2.0 다중 모드 소개

- 텍스트, 사진, 비디오 및 오디오 업로드를 지원합니다. 이러한 자료는 개체 또는 참조 개체로 사용할 수 있습니다. 모든 콘텐츠의 움직임, 특수 효과, 형태, 카메라 움직임, 캐릭터, 장면 및 사운드를 참조할 수 있습니다. 프롬프트 단어를 명확하게 작성하면 모델이 이를 이해할 수 있습니다.
- Seedance 2.0 = 다중 모드 참조 능력(모든 것을 참조할 수 있음) + 강력한 창의적 생성 + 정확한 명령 응답(훌륭한 이해)
- 자연어를 사용하여 원하는 사진과 동작을 설명하면 됩니다. 참고자료인지 편집본인지 명확하게 밝혀주세요. 자료가 많은 경우 @객체 각각이 명확하게 표시되어 있는지 확인하고 사진, 영상, 문자를 혼동하지 않는 것이 좋습니다.
...
## 늘 해결하기 어려웠던 영상 문제를 이제 정말 해결할 수 있습니다!
비디오를 만들 때 항상 몇 가지 귀찮은 측면에 직면하게 됩니다. 예를 들어 얼굴이 바뀌고, 움직임이 동일하지 않고, 비디오가 부자연스럽게 확장되고, 변경하면 전체 리듬이 바뀌는 등... 이번에는 다중 양식이 이러한 "오래되고 어려운" 문제를 한 번에 해결할 수 있습니다. 구체적인 사용 사례는 다음과 같습니다👇

### 일관성의 종합적인 개선
다음과 같은 문제가 발생했을 수 있습니다. 사진의 문자가 앞뒤로 다르게 보임, 제품 세부 정보가 손실됨, 작은 텍스트가 흐려짐, 장면이 건너뛰음, 렌즈 스타일이 통일될 수 없음... 생성 시 이러한 일반적인 일관성 문제가 이제 2.0에 있습니다.
해결될 수 있습니다. 얼굴부터 의상, 글꼴 디테일까지 전체적인 일관성이 더욱 안정적이고 정확해졌습니다.
