# 🎬 Jimeng Seedance 2.0 ユーザーマニュアル (新しいマルチモーダル作成エクスペリエンス)

テキストと最初/最後のフレームでしか「ストーリーを伝える」ことができなかった日から、私たちはあなたの表現を真に理解するビデオモデルを作りたいと思っていたことを今でも覚えています。今日、本当に来ましたよ！

Jimeng Seedance 2.0 は、画像、ビデオ、オーディオ、テキストの 4 つのモーダル入力モードをサポートし、より豊かな表現とより制御可能な生成を備えています。

画像を使用して画像のスタイルを設定し、ビデオを使用してキャラクターの動きやレンズの変更を指定し、数秒間の音声を使用してリズムと雰囲気を引き出すことができます...即興の言葉を使用すると、クリエイティブなプロセスがより自然で効率的になり、本物の「ディレクター」のようになります。

今回のアップグレードでは、「参照機能」が最大のハイライトです。
- 📷 参照画像は、写真の構図とキャラクターの詳細を正確に復元できます
- 🎥 参考ビデオは、レンズ言語、複雑なアクションのリズム、創造的な特殊効果の再現をサポートします。
- ⏱ ビデオはスムーズな拡張と接続をサポートし、ユーザーのプロンプトに従って連続映像を生成できます。世代だけでなく「撮り続ける」
- ✂️ 編集機能も同時に強化され、役割の置き換え、削除、既存のビデオの追加がサポートされます。

ビデオ制作は「生成」だけではなく、表現のコントロールも重要であることを私たちは知っています。 2.0 はマルチモーダルであるだけでなく、真に制御可能な作成方法でもあります。

Seedance 2.0、マルチモーダルな制作はここから始まります。大胆に想像して、あとはお任せください

## パラメータのプレビュー

|コア寸法|シーダンス 2.0 |
| --- | --- |
|画像入力と形式: jpeg、png、webp、bmp、tiff、gif |写真 9 枚以下<br/> サイズ: 30 MB 未満 |
|ビデオ入力と形式: mp4、mov | ≤ 3、合計継続時間 [2,15] 秒<br/> サイズ: 50 MB 未満<br/> 合計ビデオ ピクセル範囲: [409600 (640×640,480p)、927408 (834×1112,720p)] |
|オーディオ入力と形式: mp3、wav | ≤ 3、合計期間は 15 秒以内<br/> サイズ: 15MB 未満 |
|テキスト入力 |自然言語 |
|生成時間 | ≤ 15 秒、自由に 4 ～ 15 秒を選択できます。
|音声出力 |効果音/サウンドトラック付き |
|インタラクション制限: 現在サポートされている混合入力の最大合計数は 12 ファイルです。絵やリズムに最も大きな影響を与える素材を優先してアップロードし、さまざまなモードでファイル数を合理的に割り当てることをお勧めします。

## ⚠️注意: リアルな顔素材をアップロードする手順
プラットフォームのコンプライアンス要件により、リアルな素顔を含む素材のアップロードは現在サポートされていません（写真素材とビデオ素材の両方は許可されていません）。ユーザーの権利と生成のセキュリティを保護するために、システムはそのような素材を自動的にブロックし、アップロード後にビデオ コンテンツは生成されません。
これは、実際の人物の写真 (特に、はっきりと識別できる顔) をアップロードする場合、モデルは対応関係の生成を処理できないことを意味します。
これによりいくつかの制限が生じる可能性があることは承知していますが、この措置はコンテンツのセキュリティとプラットフォームの運用を確保するためのものです。皆様のご理解とご協力をよろしくお願いいたします！今後更新がある場合は、適時にドキュメントで更新します。ご理解のほどよろしくお願いいたします~
...
# マルチモーダル総合アップグレード：動画制作は「自由組み合わせ」の時代へ！

## Seedance 2.0 マルチモーダルの概要

- テキスト、写真、ビデオ、オーディオのアップロードをサポートします。これらのマテリアルは、オブジェクトまたは参照オブジェクトとして使用できます。あらゆるコンテンツの動き、特殊効果、フォルム、カメラの動き、キャラクター、シーン、サウンドを参照できます。プロンプトの単語が明確に書かれている限り、モデルはそれを理解できます。
- Seedance 2.0 = マルチモーダル参照能力 (あらゆるものを参照できる) + 強力な創造的生成 + 正確なコマンド応答 (優れた理解)
- 自然言語を使用して、必要な画像やアクションを説明するだけです。参照なのか編集なのかを明確にしてください。素材が多い場合は、各@オブジェクトが明確にマークされているか、写真、動画、文字が混同されていないか確認することをお勧めします。
...
## これまで解決が困難であったビデオの問題が、今では本当に解決できるようになりました。
動画を作ると、顔が変わる、動きが同じではない、動画が不自然に伸びる、変えると全体のリズムが変わってしまう…など、面倒な部分が必ず出てきますが、今回のマルチモダリティはそんな「昔から難しかった」問題を一気に解決します。具体的な使用例は次のとおりです👇

### 一貫性の包括的な改善
写真の文字が表と裏で違うように見える、商品の詳細が失われる、小さな文字がぼやける、シーンが飛ぶ、レンズスタイルが統一できない...これらの作成時によくある一貫性の問題は、2.0 で解決されました。
解決できる。顔から服装、フォントの詳細に至るまで、全体的な一貫性がより安定して正確になります。
...
#### 📝 プロンプト
```
0 ～ 2 秒の画面: 高速 4 フレームのフラッシュ カット、赤、ピンク、紫、ヒョウ柄の 4 つの弓が順番にフリーズし、サテンの光沢と「chéri」のブランド ワードがクローズ アップされます。ナレーション「チェリ 자석 리본으로 무궁무진한 아름다움을 연출해 보세요!」
3～6 秒: シルバーの磁気バックルが「カチッ」と音を立てて結合し、ゆっくりと引き離す様子の拡大図。シルクのような質感と利便性がわかります。ナレーション「단 1초 만에 잠그고, 최고의 스타일을 완성하세요!」
7〜12秒の画面: 着用シーンを素早く切り替えます: バーガンディスタイルはコートの襟に固定され、完全な通勤雰囲気を与えます。ピンクのスタイルはポニーテールに結ばれており、かわいい女の子は通りに出ています。パープルのスタイルはバッグのストラップに結び付けられており、ニッチでハイエンドです。スーツの襟にはヒョウ柄のスタイルがかかっており、ホットな女の子のオーラが溢れています。ナレーション「코트、가방、헤어 액세서리까지、다재다능하고 개성 넘치는 스타일을 완성하세요!」
13～15秒画面：商品名「chéri, 당신에게 즉각적인 아름다움을 선사합니다!」の4種類の弓が並んで表示されます。
```
...
#### 📝 プロンプト
```
@写真2からバッグのコマーシャル写真展示を行います。バッグの側面は@写真1を参照、バッグの表面素材は@写真3を参照します。バッグの細部まで表示する必要があり、バックグラウンドサウンドは素晴らしいです。
```
